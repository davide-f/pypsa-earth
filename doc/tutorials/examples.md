...
the rules, a [notebook folder, as part of the documentation repository](https://github.com/pypsa-meets-earth/documentation) has been provided.
...
- `build_bus_regions`: it explores the bus region shapes now created by `base_network`,
  together with the elements of the network (lines, substations, etc.)
...
- `demand_gegis`: it enables exploring th GeGIS dataset used to perform the analysis.
  These data are obtained using the [GlobalEnergyGIS](https://github.com/niclasmattsson/GlobalEnergyGIS) package for Africa.
...
- `base_network`: it eases the visualization and analysis of the output PyPSA network model and bus region shapes that the rule `base_network` builds
- `build_cutout`: the notebook analyzes the outputs of the rule `build_cutout`` rule, which are the solar, wind and hydro time series
  generated with [Atlite](https://github.com/PyPSA/atlite/)
